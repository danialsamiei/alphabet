 ...
 * @description
 * Single source of truth for AWAF API routes.
 * منبع یکتای حقیقت برای مسیرهای API در AWAF.
 *
 * The canonical API version prefix is `/api/awaf/v1`. All AWAF endpoints
 ...
 * prefix (`/api/awaf/v1/context/handshake`), and {@link normalizeApiBaseUrl}
 ...
 *
 * @example
 * import { AWAF_ROUTES, fullRoute, normalizeApiBaseUrl } from '@awaf/core';
 *
 * const base = normalizeApiBaseUrl('https://example.com/');
 * const url  = `${base}${AWAF_ROUTES.contextHandshake}`;
 */

// ─── Canonical Version Prefix ────────────────────────────────────────────────

/**
 * Canonical API version prefix for all AWAF endpoints.
 * پیشوند نسخه‌ای استاندارد برای تمام endpointهای AWAF.
 *
 ...
 */
export const API_VERSION_PREFIX = '/api/awaf/v1' as const;

/**
 * Legacy prefix accepted by clients for backwards compatibility.
 * @deprecated Use {@link API_VERSION_PREFIX} (`/api/awaf/v1`) instead.
 */
...
/**
 * Relative paths for every AWAF endpoint, keyed by stable identifier.
 *
...
export const AWAF_ROUTES = {
  ...
  // ── Visitor Interaction ───────────────────────────────────────────────────
  /** POST — Text/voice interaction with the AWAF agent (streaming or not). */
  interact: '/interact',
  ...
  // ── AWAF Pulse (formerly Technology Pulse) ────────────────────────────────
...
/** Stable key identifying an AWAF route. */
export type AwafRouteKey = keyof typeof AWAF_ROUTES;

/** A relative AWAF route path (no version prefix). */
export type AwafRoutePath = (typeof AWAF_ROUTES)[AwafRouteKey];

/**
 * Implementation status surfaced in the OpenAPI document via
 * `x-awaf-status`. Kept here so the route table can declare which
 * endpoints are wired in `@awaf/api` today and which are roadmap.
 */
export type AwafRouteStatus = 'implemented' | 'planned';

/**
 * Implementation status per route. Endpoints marked `planned` have a
 * typed request/response in `@awaf/api` but no server backing.
 */
export const AWAF_ROUTE_STATUS: Readonly<Record<AwafRouteKey, AwafRouteStatus>> = {
 ...
 *
 * @param key — A {@link AwafRouteKey}.
 * @returns The fully-qualified path, for example `/api/awaf/v1/context/handshake`.
 *
 * @example
 * fullRoute('contextHandshake'); // '/api/awaf/v1/context/handshake'
 */
export function fullRoute(key: AwafRouteKey): string {
  return `${API_VERSION_PREFIX}${AWAF_ROUTES[key]}`;
}
 ...
 * 2. **Missing prefix** — if the URL has no AWAF prefix, the canonical
 *    `/api/awaf/v1` is appended.
 ...
 *
 * @example
 * normalizeApiBaseUrl('https://x.com/');             // 'https://x.com/api/awaf/v1'
 * normalizeApiBaseUrl('https://x.com');              // 'https://x.com/api/awaf/v1'
 * normalizeApiBaseUrl('https://x.com/api');          // 'https://x.com/api'           (legacy preserved)
 * normalizeApiBaseUrl('https://x.com/api/awaf/v1');  // 'https://x.com/api/awaf/v1'
 * normalizeApiBaseUrl('https://x.com/api/awaf/v1/'); // 'https://x.com/api/awaf/v1'
 */
 ...
 * @param routePath — A route constant from {@link AWAF_ROUTES}.
 * @returns The full URL ready for `fetch`.
 */
export function joinRoute(normalizedBase: string, routePath: AwafRoutePath): string {
