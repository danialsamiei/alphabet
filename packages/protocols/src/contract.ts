/**
 * @module @awaf/protocols/contract
 * @description
 * Normalized AWAF protocol contract — the shared shape every adapter
 ...
 * layer and translate provider-specific payloads into AWAF terms:
 ...
 * the AWAF privacy policy helpers (`canStoreMemory`, `canPersonalize`, …).
 */
export type AwafConsentOperation =
  | 'read_context'
  | 'read_memory'
  | 'write_memory'
  | 'personalize'
 ...
 */
export interface AwafConsentScope {
  ...
  readonly operations: readonly AwafConsentOperation[];
 ...
 */
export interface AwafMemoryPermission {
 ...
 */
export interface AwafToolContext {
...
/**
 * Normalized AWAF request. Every adapter converts its native incoming
 ...
 */
export interface AwafProtocolRequest<TPayload = unknown> {
  /** Source protocol */
...
/**
 * Normalized AWAF response.
 *
 * @template TData - Operation-specific result data
 */
export interface AwafProtocolResponse<TData = unknown> {
  readonly correlationId: string;
  readonly protocol: ProtocolType;
  readonly operation: string;
  readonly result: Result<TData, AwafProtocolError>;
