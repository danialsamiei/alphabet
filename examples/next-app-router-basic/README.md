# AWAF — Next.js App Router basic example

> **Documentation-only snippet pack.** Next.js drags in a large dependency
> tree (>200 packages), so to keep the AWAF monorepo lightweight we ship
...
> a working AWAF integration.

## Set up a fresh Next.js project

```bash
npx create-next-app@latest my-awaf-app --ts --app --no-eslint --no-tailwind --no-src-dir --import-alias "@/*"
cd my-awaf-app
...
* `<AwafProvider>` is a Client Component (`"use client"` directive). The
  AWAF handshake reads browser-only APIs, so it can't run during the
...
1. **Install AWAF** — see `package.json` snippet above.
2. **Run handshake** — `<AwafProvider>` in `app/providers.tsx`.
...
The AWAF monorepo intentionally avoids large app-framework dependencies.
