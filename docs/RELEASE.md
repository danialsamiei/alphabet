...
This document describes the contract every Alphabet package must satisfy before
publishing, and the workflow used to cut a release.

> **Status:** Alphabet is pre-1.0 software. We do **not** publish to npm yet.
...
Every public package (`@alphabet/core`, `@alphabet/api`, `@alphabet/ui`, `@alphabet/security`,
`@alphabet/protocols`) MUST satisfy all of the following:
...
- [x] **`sideEffects: false`** — declares the package safe to tree-shake.
      Alphabet code is purely declarative; no module-load side effects.
...
pnpm size                      # size-limit tracks @alphabet/core, @alphabet/api, @alphabet/ui
pnpm size:check-r3f-free       # base @alphabet/ui bundle MUST NOT import @react-three/fiber or three
pnpm openapi:lint              # canonical /api/alphabet/v1 spec is valid OpenAPI 3.1
```
...
* The canonical API prefix `/api/alphabet/v1` is stable; the legacy `/api`
...
* Adaptive Render Layer enum values (`R3F_IMMERSIVE`, `CSS_3D`, …) are
  stable across `@alphabet/core` and `@alphabet/ui`.
...
2. Install one of the `examples/` apps against the freshly packed tarballs
   (`pnpm install ./alphabet-core-1.0.0.tgz …`) to verify resolution end-to-end.
3. Confirm `docs/PERFORMANCE.md` budgets still match
   `benchmarks/*.bench.ts`.
4. Confirm `openapi/alphabet.v1.yaml` lints cleanly with `pnpm openapi:lint`.
...
* `packages/create-alphabet` — currently planned/private (see its README).
